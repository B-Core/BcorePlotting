...
function(norm_x, p_mat, q_list, metadata, plotdata, plot2file = TRUE,
  ...
  #' @description Creates plots for each design factor (default behavior), or for factors facSel
  ...
  #' @param norm_x abundance data input into regression (eg, regressMatrix()). nrow(norm_x)==length(p_mat)
  #' @param p_mat matrix of p-values for each factor. Returned by regressMatrix()
  #' @param q_list S3 object; list of p-value lists for each factor. Returned by regressMatrix() 
  #' @param metadata list of factors; each list element is a factor (eg, sex = c("m", "f"...)) #Feedback perhaps as.factor(c("m","f"))?
  #'             Vector order same as norm_x column order
  #'             was attribs previously
  #' @param plotdata is a list of info relevant to labeling and saving the plot With the following elements:
  ...
  #' @param plot2file logical. if TRUE, write files using plotdata, otherwise to studio graphics viewer
  #' @param bonusMDS logical. if TRUE plots an MDS of data at qcut (see below). Default is FALSE (no plot)
  #' @param oneclass metadata vector for labeling bonusMDS data; if NULL or not specified, defaults to the first element of metadata
  #' @param qcut either a number between 0 and 1 used as an upper qvalue limit for MDS, OR a number >1, assumed to be the top n qvalues to use in MDS
  #' @param histbins number of bins for p-value histogram; may want to change if there is suspicious behavior
  #' @param colorspec optional range of colors for MDS plot. Defaults to hotpink/green!
  #' @param facSel optional vector of experimental factors to plot (rather than all factors) #Feedback to plot for the MDS plot? What if you want to plot based on second element of metadata list?
  #' @param filesep string that adds to end of directory path.
  #' @return Writes plots to files or to Rstudio viewer. Notes - Looking for even descent to pi0 in top left tuning plot. The slow/steep rise in q-values
...
function (norm_x, pvalue_v, obj_qvalue, attribs, oneclass, plotdata, 
                      ...
                      filesep='/') {
  #' Checks proper running of q-value analysis
  #' @description Creates plots for each design factor (default behavior), or for factors facSel
  #' 1) Histogram of p-values that were included in the design. Must not have right peak.
  #' 2) qvalue's default plots with full q-value range (c(0,1)).
  #' 3) Optional MDS plot at q-value cut
  #' @param norm_x abundance data input into regression (eg, regressMatrix()). nrow(norm_x)==length(p_mat)
  #' @param pvalue_v vector of p-values previously used as input to qvalue()
  #' @param obj_qvalue: S3 object of qvalue class (a list!) returned by qvalue()
  ...
  #'     plotSubtitle:  optional subtitle; suggest lm_expression
  #' @param colorspec optional range of colors for MDS plot. Defaults to hotpink/green!
  #' @param qcut either a number between 0 and 1 used as an upper qvalue limit for MDS,
  #'     OR a number >1, assumed to be the top n qvalues to use in MDS
  #' @param histbins number of bins for p-value histogram; may want to change if there is suspicious behavior
  #' @param plot2file logical. if TRUE, write files using plotdata, otherwise to studio graphics viewer
  #' @param p_hist logical. TRUE (default) to plot this type. 
  #' @param q_plots logical. TRUE (default) to plot this type.
  #' @param MDS_plot logical. TRUE to plot this type. FALSE is default. Will plot after selecting q-value cut based on QC plots. Use plotRatios() instead!
  #' @param filesep string that adds to end of directory path.
  #' @return Writes plots to files or to Rstudio viewer. Notes - Looking for even descent to pi0 in top left tuning plot. The slow/steep rise in q-values
  #' in remaining plots depends on the resolving power of the data.
...
function (normmat, rawmat, attribs, oneclass, plotdata,
  ...
  #' Creates a heatmap and an MDS plot
  #' @description This function currently only considers a single clustering variable and will need to be generalized.
  #' Data will be centered around the average of all data in the normalized data set
  #' As a QC plot, want to filter out non-changing data, otherwise it takes a LONG time to complete
  #' @param normmat data matrix
  #' @param rawmat  raw data matrix (optionally used to center normmat)
  #' @param attribs  list of sample classifications to be tracked in clustering 
  #' each list element contains a string vector with one label per sample
  #' @param oneclass string name of attribs element to be used in MDS plot
  #' @param plotdata is a list of info relevant to labeling and saving the plot
  #'    plotdir:  plot destination directory
  #'    plotbase:  base filename for the plot
  #'    plottitle:  title for all plots
  #' @param  colorspec is a vector of color specifiers for colorRampPalette  
  #' @param  center  center data on 'norm' for normalized average
  ...
  #' @param  mask  NULL default masks rows with SD > SD of entire data set; 
  #'         Numeric vector of length 1 masks rows with SD > value
  #'         Vector of length nrows(normmat) is a custom mask   ***** TBImplemented
  #' @param  plot2file  if true, plot to file as specified in plotdata
